...
define(`IMAGEVERSION',patsubst(patsubst(DOTTAG,`R\.',`'),`\.branch',`'))dnl
...
#
`#' docker build --rm -t shabbychef/`rbranch:'IMAGEVERSION . 
changequote(`<<<',`>>>')dnl
#
# Created: 2016.05.01
...
RUN (mkdir -p /tmp/build ; \
  cd /tmp/build ; \
  svn co http://svn.r-project.org/R/branches/DASHTAG/ Rsrc ; \
  cd /tmp/build/Rsrc ; \
    R_PAPERSIZE=letter \
